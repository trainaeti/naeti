class CourseDate < ActiveRecord::Base
  ...
  scope :available, where("class_size >= ?", OrderItem.where(:course_date_id=>self.id).count) 
  scope :waiting, where("class_size < ?", OrderItem.where(:course_date_id=>self.id).count) 
